...
def edit_shill(update : Update, context : CallbackContext):
    context.bot.send_message(update.effective_message.chat_id,"select the shill post to edit 👇",reply_markup=edit_shill_post_markup())
    return POST
...
def add_post_1(update : Update, context : CallbackContext):
    ...
    context.bot.send_message(update.message.chat.id,update.message.text,parse_mode=ParseMode.HTML)
    context.bot.send_message(update.message.chat.id,"Done",reply_markup=admin_markup())
...
def shill1(update : Update,context : CallbackContext):
    ...
    context.bot.send_message(update.effective_message.chat_id,data,disable_web_page_preview=True,parse_mode=ParseMode.HTML)
...
def set_welcome_message(update : Update,context : CallbackContext):
    context.bot.send_message(update.effective_message.chat_id,"Send a welcome text [ Use HTML tags for blod,italic]",reply_markup=cancel_markup())
    return WELCOME
...
EDIT_SHILL_POST_HANDLER=ConversationHandler(
    entry_points=[MessageHandler(Filters.regex('^📝 Edit Shill Post$'),edit_shill)],
    states={
        POST:[
        ...
        SHILL3:[MessageHandler(Filters.text & ~Filters.regex('^🚫 Cancel$'),add_post_3)]
...
SET_WELCOME_TEXT_HANDLER=ConversationHandler(
    entry_points=[MessageHandler(Filters.regex('^✉️ Set Message$'),set_welcome_message)],
    states={
        WELCOME : [MessageHandler(Filters.text & ~Filters.regex('^🚫 Cancel$'),process_welcome_text)]
    },
    fallbacks=[
        MessageHandler(Filters.regex('^🚫 Cancel$'),cancel_shill)]
)
...
SET_WELCOME_IMAGE_HANDLER=ConversationHandler(
    entry_points=[MessageHandler(Filters.regex('^🖼 Set Image$'),set_welcome_image)],
    states={
        IMAGE : [MessageHandler(Filters.regex(r'^https:\/\/ibb.co\/[0-9A-Za-z]+$'),process_welcome_image)]
    },
    fallbacks=[
        MessageHandler(Filters.regex('^🚫 Cancel$'),cancel_shill)]
)
